...
class Experiment(object):
    ...
    def predict(self, X: torch.Tensor):
        ...
        """
        with torch.no_grad():
            output = self.ckpt.model(X)
            prediction = torch.argmax(nn.functional.softmax(output, dim=-1)) # PROBABLY WRONG
        return prediction
    ...
    def step(self, X: torch.Tensor, y: torch.Tensor, epoch: int, iteration: int)->Tuple[float, torch.Tensor]:
        ...
        """
        output = self.ckpt.model(X)
        loss = self.ckpt.criterion(output, y)
        ...
        return train_loss, output.detach()
